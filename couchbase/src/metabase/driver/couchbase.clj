...
(driver/register! :couchbase, :parent :sql-jdbc)
...
(doseq [[feature supported?] {:regex                                  false
                              ...
                              :index-info                             true}]
  (defmethod driver/database-supports? [:couchbase feature] [_driver _feature _db] supported?))

(defmethod driver/database-supports? [:couchbase :percentile-aggregations]
  [_ _ db]
  (let [major-version (get-in db [:dbms_version :semantic-version 0] 0)]
    (when (zero? major-version)
      (log/warn (trs "Unable to determine couchbase's dbms major version. Fallback to 0.")))
    (>= major-version 16)))

(defmethod driver/db-start-of-week :couchbase
  [_]
  :sunday)

(defmethod driver/prettify-native-form :couchbase
  [_ native-form]
...
(defmethod sql-jdbc.sync/database-type->base-type :couchbase
  [_ column-type]
...
(defmethod sql-jdbc.conn/connection-details->spec :couchbase
  [_ {:keys [user password db host port] :or   {user "dbuser", password "dbpassword", db "", host "localhost", port 8094}}]
      {:classname         "com.couchbase.client.jdbc.CouchbaseDriver"
       :subprotocol        "cb:analytics"
...
(defmethod sql.qp/->honeysql [:couchbase :field]
...
(defmethod sql.qp/date [:couchbase :default]
  [_driver _unit expr]
  expr)

(defmethod sql.qp/date [:couchbase :second-of-minute]
  [_driver _unit expr]
...
(defmethod sql.qp/date [:couchbase :minute]
  [_driver _unit expr]
...
(defmethod sql.qp/date [:couchbase :minute-of-hour]
  [_ _ expr]
...
(defmethod sql.qp/date [:couchbase :hour]
  [_driver _unit expr]
  (if (= (h2x/database-type expr) "time")
...
(defmethod sql.qp/date [:couchbase :hour-of-day]
  [_driver _unit expr]
  (date-part :hour expr))

(defmethod sql.qp/date [:couchbase :day]
  [_driver _unit expr]
...
(defmethod sql.qp/date [:couchbase :day-of-week]
  [_driver _unit expr]
  (sql.qp/adjust-day-of-week :couchbase (date-part :weekday expr)))

(defmethod sql.qp/date [:couchbase :day-of-month]
  [_driver _unit expr]
  (date-part :day expr))

(defmethod sql.qp/date [:couchbase :day-of-year]
  [_driver _unit expr]
...
(defmethod sql.qp/date [:couchbase :week]
  [driver _ expr]
  (sql.qp/adjust-start-of-week driver trunc-week expr))

(defmethod sql.qp/date [:couchbase :week-of-year-iso]
  [_ _ expr]
  (date-part :iso_week expr))

(defmethod sql.qp/date [:couchbase :month]
  [_ _ expr]
  (if (::optimized-bucketing? *field-options*)
    (h2x/month expr)
    [:DateFromParts (h2x/year expr) (h2x/month expr) [:inline 1]]))

(defmethod sql.qp/date [:couchbase :month-of-year]
  [_driver _unit expr]
...
(defmethod sql.qp/date [:couchbase :quarter]
  [_driver _unit expr]
  (date-add :quarter
            (h2x/dec (date-part :quarter expr))
            [:DateFromParts (h2x/year expr) [:inline 1] [:inline 1]]))

(defmethod sql.qp/date [:couchbase :quarter-of-year]
  [_driver _unit expr]
  (date-part :quarter expr))

(defmethod sql.qp/date [:couchbase :year]
  [_driver _unit expr]
  (if (::optimized-bucketing? *field-options*)
    (h2x/year expr)
    [:DateFromParts (h2x/year expr) [:inline 1] [:inline 1]]))

(defmethod sql.qp/date [:couchbase :year-of-era]
  [_driver _unit expr]
  (date-part :year expr))

(defmethod sql.qp/add-interval-honeysql-form :couchbase
  [_ hsql-form amount unit]
  (date-add unit amount hsql-form))

(defmethod sql.qp/unix-timestamp->honeysql [:couchbase :seconds]
  [_ _ expr]
...
(defmethod sql.qp/->honeysql [:couchbase :convert-timezone]
...
(defmethod sql.qp/->honeysql [:couchbase :datetime-diff]
  [driver [_ x y unit]]
...
(defmethod sql.qp/datetime-diff [:couchbase :year]
  [driver _unit x y]
  (h2x// (sql.qp/datetime-diff driver :month x y) 12))

(defmethod sql.qp/datetime-diff [:couchbase :quarter]
  [driver _unit x y]
  (h2x// (sql.qp/datetime-diff driver :month x y) 3))

(defmethod sql.qp/datetime-diff [:couchbase :month]
  [_driver _unit x y]
...
(defmethod sql.qp/datetime-diff [:couchbase :week] [_driver _unit x y] (h2x// (date-diff :day x y) 7))
(defmethod sql.qp/datetime-diff [:couchbase :day] [_driver _unit x y] (date-diff :day x y))
(defmethod sql.qp/datetime-diff [:couchbase :hour] [_driver _unit x y] (h2x// (date-diff :millisecond x y) 3600000))
(defmethod sql.qp/datetime-diff [:couchbase :minute] [_driver _unit x y] (date-diff :minute x y))
(defmethod sql.qp/datetime-diff [:couchbase :second] [_driver _unit x y] (date-diff :second x y))

(defmethod sql.qp/cast-temporal-string [:couchbase :Coercion/ISO8601->DateTime]
  [_driver _semantic_type expr]
  (h2x/->datetime expr))

(defmethod sql.qp/cast-temporal-string [:couchbase :Coercion/YYYYMMDDHHMMSSString->Temporal]
  [_driver _semantic_type expr]
...
(defmethod sql.qp/apply-top-level-clause [:couchbase :breakout]
...
(defmethod sql.qp/apply-top-level-clause [:couchbase :order-by]
...
;; couchbase doesn't support `TRUE`/`FALSE`; it uses `1`/`0`, respectively; convert these booleans to numbers.
(defmethod sql.qp/->honeysql [:couchbase Boolean]
  [_ bool]
  (if bool 1 0))

(defmethod sql.qp/->honeysql [:couchbase Time]
  [_ time-value]
  (h2x/->time time-value))

(defmethod sql.qp/->honeysql [:couchbase :stddev]
  [driver [_ field]]
  [:stdevp (sql.qp/->honeysql driver field)])

(defmethod sql.qp/->honeysql [:couchbase :var]
  [driver [_ field]]
  [:varp (sql.qp/->honeysql driver field)])

(defmethod sql.qp/->honeysql [:couchbase :substring]
...
(defmethod sql.qp/->honeysql [:couchbase :length]
  [driver [_ arg]]
  [:len (sql.qp/->honeysql driver arg)])

(defmethod sql.qp/->honeysql [:couchbase :ceil]
  [driver [_ arg]]
  [:ceiling (sql.qp/->honeysql driver arg)])

(defmethod sql.qp/->honeysql [:couchbase :round]
  [driver [_ arg]]
  [:round (h2x/cast :float (sql.qp/->honeysql driver arg)) 0])

(defmethod sql.qp/->honeysql [:couchbase :power]
  [driver [_ arg power]]
...
(defmethod sql.qp/->honeysql [:couchbase :percentile]
  [driver [_ arg val]]
  [::approx-percentile-cont
   (sql.qp/->honeysql driver arg)
   (sql.qp/->honeysql driver val)])

(defmethod sql.qp/->honeysql [:couchbase :median]
  [driver [_ arg]]
...
;; [[metabase.driver.couchbase-test/filter-by-datetime-fields-test]]).
...
(defmethod sql.qp/->honeysql [:couchbase OffsetTime]
  [_driver t]
  (cond-> t
...
(doseq [c [OffsetDateTime ZonedDateTime]]
  (defmethod sql.qp/->honeysql [:couchbase c]
    [_driver t]
    (cond-> t
            (timezoneless-comparison?) (format-without-trailing-zeros datetime-format))))

(doseq [op [:= :!= :< :<= :> :>= :between]]
  (defmethod sql.qp/->honeysql [:couchbase op]
...
(defmethod driver/db-default-timezone :couchbase
  [driver database]
...
(defmethod sql.qp/current-datetime-honeysql-form :couchbase
  [_]
  (h2x/with-database-type-info :%getdate "datetime"))

(defmethod sql-jdbc.sync/excluded-schemas :couchbase
  [_]
...
(defmethod sql.qp/preprocess :couchbase
  [driver inner-query]
...
;; statements (see #9940)
(defmethod sql-jdbc.execute/statement-supported? :couchbase [_]
  false)
...
;; and
;; https://github.com/microsoft/mssql-jdbc/blob/v9.2.1/src/main/java/com/microsoft/couchbase/jdbc/couchbaseConnection.java#L5349-L5357
...
(defmethod sql-jdbc.execute/prepared-statement :couchbase
...
(defmethod sql-jdbc.execute/statement :couchbase
  [_ ^Connection conn]
...
(defmethod unprepare/unprepare-value [:couchbase LocalDate]
  [_ ^LocalDate t]
...
(defmethod unprepare/unprepare-value [:couchbase LocalTime]
  [_ ^LocalTime t]
...
(defmethod unprepare/unprepare-value [:couchbase OffsetTime]
  [driver t]
  (unprepare/unprepare-value driver (t/local-time (t/with-offset-same-instant t (t/zone-offset 0)))))

(defmethod unprepare/unprepare-value [:couchbase OffsetDateTime]
  [_ ^OffsetDateTime t]
...
(defmethod unprepare/unprepare-value [:couchbase ZonedDateTime]
  [driver t]
  (unprepare/unprepare-value driver (t/offset-date-time t)))

(defmethod unprepare/unprepare-value [:couchbase LocalDateTime]
  [_ ^LocalDateTime t]
...
;; this mystery out
(defmethod sql-jdbc.execute/set-parameter [:couchbase OffsetTime]
  [driver ps i t]
...
(defmethod sql-jdbc.execute/read-column-thunk [:couchbase microsoft.sql.Types/DATETIMEOFFSET]
  [_^ResultSet rs _ ^Integer i]
  (fn []
    (.getObject rs i OffsetDateTime)))

;; SQL Server doesn't really support boolean types so use bits instead (#11592)
(defmethod driver.sql/->prepared-substitution [:couchbase Boolean]
  [driver bool]
  (driver.sql/->prepared-substitution driver (if bool 1 0)))

(defmethod driver/normalize-db-details :couchbase
  [_ database]
